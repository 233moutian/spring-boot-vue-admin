 ...
 * JedisUtil 用于redis的各种操作
 * 每一次使用都会自动从池中获取,然后自己的close.不需要自己管.set方法调用之后会返回此操作的状态,OK等
 */
public class JedisUtil {
    ...
    /**缓存过期时间一个月即30*24小时**/
    public final int EXPIRE_ONEMONTH = 2592000;//60*60*24*30
    /**缓存过期时间十天即10*24小时**/
    public final int EXPIRE_TENDAY = 864000;//60*60*24*10
    /**过期时间为一周即7*24小时**/
    public static int EXPIRE_ONEWEEK = 604800;//7*60*60*24
    /**缓存过期时间一天即24小时**/
    public final int EXPIRE_ONEDAY = 86400;//60*60*24
    /**缓存过期时间半天即12小时**/
    public final int EXPIRE_HALFDAY = 43200;//60*60*12
    /**缓存过期时间三小时即3*60分钟**/
    public final int EXPIRE_THREEHOUR = 10800;//3*60*60
    /**缓存过期时间一小时即60分钟**/
    public final int EXPIRE_ONEHOUR = 3600;//60*60
    /**缓存过期时间半小时即30分钟**/
    public final int EXPIRE_HALFHOUR = 1800;//60*30
    /**缓存过期时间十分钟**/
    public final int EXPIRE_TENMINUTE = 600;//60*10
    /**缓存过期时间90秒**/
    public final int EXPIRE_NINETY = 90;
    /**缓存过期时间半分钟即30秒**/
    public final static int EXPIRE_HALFMINUTE = 30;
    ...
    public class Sets {
        ...
        public List<byte[]> srandmember(byte[] key, int count) {
            Jedis jedis = getJedis();
            List<byte[]> set = jedis.srandmember(key, count);
            returnJedis(jedis);
            return set;
        }

        public List<String> srandmember(String key) {
            return srandmember(key, 1);
        }
    ...
    public class Strings {
         ...
         * @return 存在此Key返回TRUE
         */
        public boolean exisit(String key) {
            Jedis jedis = getJedis();
            long str = jedis.setnx(key, "");
            returnJedis(jedis);
            if (str == 0) {
                return true;
            }
